...
vector<vector<string>> read_input(const string& path) {// TODO:[1] read transactions from xlsx file and return transactions
    //    @tarek and @abdo
    ifstream file(path);
...
vector<freq> get_c1_frequent(const vector<vector<string>>& transactions, int support) {// TODO:[2] read transactions from xlsx file and return transactions
    //  @anas

    unordered_map<string, int> item_freq = get_item_frequencies(transactions);
    vector<freq> c1_frequent;

    for (auto &it: item_freq) {
...
vector<vector<string>> rebuild_transactions(const vector<vector<string>>& transactions, const vector<freq>& c1_frequent, int support) {// TODO:[3] read transactions from xlsx file and return transactions
    //  @abdo
    ...
    for (const auto &transaction: transactions) {
        ...
        sorted_transaction.erase(remove(sorted_transaction.begin(), sorted_transaction.end(), ""),
